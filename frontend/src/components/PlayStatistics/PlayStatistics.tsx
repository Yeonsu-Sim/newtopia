...
export default function PlayStatistics({ series, finalTurnNumber, gameId }: Props) {
  const [selectedMetric, setSelectedMetric] = useState<"all" | "economy" | "environment" | "publicSentiment" | "defense">("all");
  const [selectedTurn, setSelectedTurn] = useState<number | null>(null);

  const { data: turnDetail } = useGameResultTurnDetail(gameId, selectedTurn ?? 0);
  ...
  const filteredSeries = selectedMetric === "all"
    ? series
  ...
  return (
    <Wrapper>
      ...
      <Section>
        ...
        <div style={{ minWidth: `${filteredSeries[0]?.points.length * 50}px` }}>
            <ReportLineChart
      ...
      {turnDetail && (
        <DetailSection>
          <DetailHeader>{turnDetail.data.context.turnNumber}턴 상세 정보</DetailHeader>
          ...
          <ChoiceWrapper>
            <Label>당신의 선택!</Label>
            <ChoiceCards>
                <ChoiceCard type="A" choiceCode={choiceCode}>{turnDetail.data.card.choices[0].label}</ChoiceCard>
                <ChoiceCard type="B" choiceCode={choiceCode}>{turnDetail.data.card.choices[1].label}</ChoiceCard>
            </ChoiceCards>
          </ChoiceWrapper>
          ...
          <GraphWrapper>
            ...
            <StatsGraphWrapper style={{ height: 200 }}>
              {Object.keys(turnDetail.data.applied.countryStats.before).map((metric) => {
                const before = turnDetail.data.applied.countryStats.before[metric];
                const after = turnDetail.data.applied.countryStats.after[metric];
                const delta = turnDetail.data.applied.countryStats.delta[metric];
                ...
                return (
                  <MetricColumn key={metric}>
                    ...
                    <span style={{ fontSize: "12px" }}>{before} → {after}</span>
                  </MetricColumn>
                )
              })}
            </StatsGraphWrapper>
          ...
          <ArticleWrapper>
            <Label>연관 뉴스</Label>
            <ArticleTitle>{turnDetail.data.card.relatedArticle.title}</ArticleTitle>
            <ArticleContent>{turnDetail.data.card.relatedArticle.content}</ArticleContent>
            <ArticleUrl><a href={turnDetail.data.card.relatedArticle.url} target="_blank">원본 기사 보러가기!</a></ArticleUrl>
          </ArticleWrapper>
        </DetailSection>
      )}
    </Wrapper>
