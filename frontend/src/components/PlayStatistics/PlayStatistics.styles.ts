...
export const MetricButton = styled.button<MetricButtonProps>`
  ...
  &:hover {
    background: ${({
      $active,
      $activeHoverColor = '#00695c',
      $inactiveHoverColor = '#d5d5d5',
...
export const ChoiceCard = styled.button<ChoiceCardProps>`
  ...
  color: ${({ type, choiceCode }) =>
    type === choiceCode ? 'white' : '#dddddd'};
  font-size: ${({ type, choiceCode }) =>
    type === choiceCode ? '28px' : '22px'};
  font-weight: bold;
  ...
  height: ${({ type, choiceCode }) =>
    type === choiceCode ? '350px' : '280px'};
  padding: 20px 40px;
  border-radius: 12px;
  font-family: 'DNFBitBitv2', 'Noto Sans KR', sans-serif;
  transition: all 0.2s ease;
