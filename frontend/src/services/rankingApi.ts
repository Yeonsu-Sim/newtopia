...
// 상위 N개 랭킹 조회 (통합랭킹)
export const getTopRanking = async (topN: number = 100): Promise<RankingItem[]> => {
  const response = await fetch(`${API_BASE_URL}/public/ranking/top?topN=${topN}`, {
    method: 'GET',
    headers: {
      'Content-Type': 'application/json',
    },
...
export const getGameRanking = async (gameId: number): Promise<RankingItem> => {
  const response = await fetch(`${API_BASE_URL}/public/ranking/gameId/${gameId}`, {
    method: 'GET',
    headers: {
      'Content-Type': 'application/json',
    },
